  ...
  return (
    <div className="w-full flex flex-col">
      ...
      <section id="console" className="relative py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full scroll-mt-20">
        ...
        {data ? (
          <LiveBookContent data={data} live={live} error={error} />
        ) : (
...
function LiveBookContent({ data, live, error }: { data: any; live: boolean; error: string | null }) {
...
function Unavailable({ error }: { error: string | null }) {
  return (
    <GlowingCard className="text-center py-12 border-dashed border-[#231F42]">
      ...
      <h3 className="text-lg font-bold text-white">Live Backend Stream Standby</h3>
      <p className="text-sm text-[#94A3B8] max-w-md mx-auto mt-2 leading-relaxed">
        {error ?? "FastAPI risk engine is in standby. To stream live brokerage accounts, start the local backend with your Supabase credentials."}
      </p>
